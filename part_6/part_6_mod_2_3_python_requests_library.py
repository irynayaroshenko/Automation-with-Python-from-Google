...
# That's it! That was a basic request for a web page! We used the Requests library to make HTTP GET request for a
# specific URL, or Uniform Resource Locator. The URL tells the Requests library the name of resource (www.google.com)
# and what protocol to use to get the resource (https://). The result we get is an object of type requests.Response
...
# https://requests.readthedocs.io/en/master/api/#requests.Response.text :
...
# Now, it might be hard for you to read the HTML (HyperText Markup Language) https://html.spec.whatwg.org/multipage/
...
# code to find the web server, make a network connection, construct an HTTP message, wait for a response, or decode the
# response. Not that HTML can't be messy enough on its own, but let's look at the first bytes of the raw
# https://requests.readthedocs.io/en/master/api/#requests.Response.raw
...
# What's all that? The response was compressed with gzip https://www.gzip.org/
...
#
# The requests.Response https://requests.readthedocs.io/en/master/api/#requests.Response
# object also contains the exact request that was created for us. We can check out the headers stored in our object to
# see that the Requests module told the web server that it was okay to compress the content:
