...
# developers write a Python module that they think others might find useful, they publish it in PyPI -- also known as
# Python Package Index (https://pypi.org). We can browse this repository of Python modules to find the module we need.
# It includes thousands of projects, which are classified by different categories, like topic, development status, and
# intended audience.

# In this module, we’re going to be transforming and converting images. To do that, we'll be using a popular library for
# image manipulation: the Python Imaging Library (PIL). The original PIL library hasn't been updated since 2009 and does
# not support Python 3. Fortunately, there's a current fork of PIL called Pillow https://pypi.org/project/Pillow/
# It properly supports Python 3 and is kept up-to-date. The Pillow library is packaged with the name 'pillow', but the
# module name in Python is still PIL.
...
# Okay, looks like I don't have that module yet! As we covered in an earlier course
# https://www.coursera.org/learn/python-operating-system/lecture/NxUXx/getting-your-computer-ready-for-python , there
# are several ways to add external modules to your Python environment. PIL is a pretty common library, and on Linux it’s
# usually available as a native package. For example:
...
# library's Application Programming Interface (API)!
...
# provide APIs in the form of external or public functions, classes, and methods that other code can use to get their
# job done without having to create a lot of repeated code.
...
# code. These internal or private functions, classes, and methods do important work, but they’re there to support the
# functions that are published by the library. You probably don't have time to dig in to understand every little bit of
# how the code works, but you need to know how to interact with the library to do useful work. An API is sort of like a
# promise. Even if the library's internal code changes, you expect the function to keep accepting the same parameters
# and returning the same results. That provides a stable interface to write your code with. That's an API!
#
...
# depends on that library. When a library author needs to make a breaking change to an API, then they need to have a
# plan in place for communicating that change to their users. That's why breaking changes should be saved for major
# version increments of a library https://semver.org/#summary (Semantic Versioning 2.0.0)
...
# For example, when we visit the reference page for the Image object https://pillow.readthedocs.io/en/stable/reference/Image.html
...
# supposed to do. When dealing with open-source libraries, we can even check out how the function is implemented to see
# if it matches our expectations. For a web service API or a closed-source library, you might not have access to the
# underlying code, but you should have access to the documentation that’s generated by the code.
...
# There, the docstrings have been compiled into a browsable reference, and they’ve also written a handbook
# https://pillow.readthedocs.io/en/stable/handbook/index.html
#  with tutorials https://pillow.readthedocs.io/en/stable/handbook/tutorial.html
#  for you to get familiar with the library's API.
