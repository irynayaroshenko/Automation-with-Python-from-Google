...
to use the transform_record function to do that. result = re.sub(r'^([\w .-]*), ([\w .-]*)$', r'\2 \1', 'Yaroshenko, Ira')
"""
def transform_record(record):
    new_record = re.sub(r'(\d+-[\d-]+)', r'+1-\1', record)
    # print(re.search(r'\d+-[\d-]+', record))
    return new_record
...
This means looking for text that begins with a hash mark (#) and replacing it with double slashes (//), which is
the C single-line comment indicator. For the purpose of this exercise, we'll ignore the possibility of a hash mark
embedded inside of a Python command, and assume that it's only used to indicate a comment. We also want to treat
repetitive hash marks (##), (###), etc., as a single comment indicator, to be replaced with just (//) and not (#//) or
(//#). Fill in the parameters of the substitution method to complete this function: 
"""
def transform_comments(line_of_code):
    result = re.sub(r'\w?#+', r'//', line_of_code)
    # result = re.search(r'\w?#+', line_of_code)
    if result is None:
        return line_of_code
    return result
...
def convert_phone_number(phone):
    result = re.sub(r'(\b\d{3})-(\d{3}-\d{4}\b)', r'(\1) \2', phone)  # my solution. I decide where to make groups in regex
    # result = re.search(r'\b\d{3}-\d{3}-\d{4}\b', phone)
    return result
