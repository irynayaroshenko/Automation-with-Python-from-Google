...
the information in a nicely formatted block. Fill in the gaps of the contents_of_file function to turn the data
...
def contents_of_file(filename):
    ...
    with open(filename) as flowers:
        ...
        # Process each row
        for row in rows:
            name, color, flow_type = row
            # Format the return string for data rows only
            if line_count == 0:
                line_count += 1
            else:
                return_string += "a {} {} is {}\n".format(color, name, flow_type)
                line_count += 1
    return return_string
